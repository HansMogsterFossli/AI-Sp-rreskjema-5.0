...
export default function ChatUI({ questions, onComplete }) {
  ...
  useEffect(() => {
    setMessages([
      ...
      { who: "bot", text: questions[0]?.label || "Klar?" }
    ]);
  }, []);
  ...
  async function submit(value, optionalComment) {
    ...
    setBotTyping(true);
    const nextLabel = questions[nextStep]?.label ?? "neste spørsmål";
    ...
    setMessages((m) => [...m, { who: "bot", text: ack }, { who: "bot", text: nextLabel }]);
    setStep(nextStep);
    setTimeout(scrollToBottom, 50);
  }
    ...
    <div className="card">
      ...
      {!isDone && current && (
        <div className="inputBar">
          {current.type === "text" && (
            <input className="input" value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Skriv her…" />
          )}
